...
use parami_primitives::{BlockNumber, Hash};
use parami_runtime::{
...
pub const SPEC_VERSION: u32 = parami_runtime::VERSION.spec_version;

pub const TRANSACTION_VERSION: u32 = parami_runtime::VERSION.transaction_version;

pub type TestExternalities<H> = CoreTestExternalities<H, u64>;

pub fn sign(xt: CheckedExtrinsic) -> UncheckedExtrinsic {
    parami_testing::keyring::sign(xt, SPEC_VERSION, TRANSACTION_VERSION, GENESIS_HASH)
}
...
pub fn new_test_ext(code: &[u8], support_changes_trie: bool) -> TestExternalities<BlakeTwo256> {
    let mut ext = TestExternalities::new_with_code(
        code,
        parami_testing::genesis::config(support_changes_trie, Some(code))
            .build_storage()
            .unwrap(),
    );
