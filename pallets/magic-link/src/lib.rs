...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		PreparedControllerChanging(T::AccountId, T::AccountId, T::AccountId, Option<T::AccountId>),
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_stable_account(
			...
			ensure!(controller_account != magic_account, Error::<T>::ControllerEqualToMagic);
		...
		pub fn change_controller(
			...
			ensure!(new_controller != magic_account, Error::<T>::ControllerEqualToMagic);
		...
		pub fn activate_controller(
			...
			let mut sa = StableAccounts::<T>::get(&old_controller)
				...
				sa.new_controller_account.clone().ok_or(Error::<T>::NewControllerNotFound)? ==
					new_controller,
		...
		pub fn codo(
			...
			let sa = StableAccounts::<T>::get(controller_account)
				.ok_or(Error::<T>::StableAccountNotFound)?;
			ensure!(sa.new_controller_account.is_none(), Error::<T>::NeedActivateController);
