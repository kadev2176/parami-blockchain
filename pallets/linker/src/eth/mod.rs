...
mod tests {
    ...
    fn correct_recover() {
        ...
        let address_bytes = decode("Fe7cef4F3A7eF57Ac2401122fB51590bfDf9350a").unwrap();
        let mut address = [0u8; 20];
        address[0..20].copy_from_slice(&address_bytes[0..20]);

        let addr = address_from_sig(msg, sig).ok().unwrap();
    ...
    fn msg_with_unexpected_length() {
        let msg = b"Link Parami: 0123456789abcdef0123456789abcdef999".encode();
        assert_eq!(
            Err("Unexpected ethereum message length!"),
            eth_data_hash(msg)
        );
    ...
    fn msg_with_expected_length() {
        let msg = b"Link Parami: 0123456789abcdef0123456789abcdef9999".encode();
