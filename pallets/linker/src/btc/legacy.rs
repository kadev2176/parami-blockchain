...
pub fn btc_address_from_pk(pk: &[u8]) -> [u8; 25] {
...
mod tests {
    ...
    #[test]
    fn correct_btc_address_from_pk() {
        let pk = decode("0450863AD64A87AE8A2FE83C1AF1A8403CB53F53E486D8511DAD8A04887E5B23522CD470243453A299FA9E77237716103ABC11A1DF38855ED6F2EE187E9C582BA6").unwrap();
        let mut pk_input = [0u8; 65];
        pk_input[0..65].copy_from_slice(&pk[0..65]);

        let addr = btc_address_from_pk(&pk_input);

        let address_hex = decode("00010966776006953D5567439E5E39F86A0D273BEED61967F6").unwrap();
        let mut address = [0u8; 25];
        address[0..25].copy_from_slice(&address_hex[0..25]);
